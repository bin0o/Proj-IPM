...
function drawTarget(i)
{
  ...
  if (trials[current_trial] === i && trials[current_trial] === trials[current_trial+1]) 
  { 
    //Highlights the
    ...
    // if the target is the same twice in a row, writes 2x inside of the target
    fill(color(255,255,255)); 
    textAlign(CENTER);
    textSize(40);
  ...
  { 
    ...
    stroke(color(220,220,220));
    strokeWeight(2);
    fill(color(0,204,102));             //cor do alvo para clicar    
    circle(target.x, target.y, target.w);
    
    stroke(color(0,204,102));
    strokeWeight(10);
    line(next_target.x,next_target.y,target.x,target.y);
  }
  // Check whether this target is the next target the user should be trying to select
  else if (trials[current_trial+1] === i) 
  { 
    // Highlights the next target the user should be trying to select
    noStroke();
    fill(color(0,255,247));     //cor do alvo seguinte            
    circle(target.x, target.y, target.w);
  ...
  // Draws the target
  fill(color(0,0,72));    //cor dos alvos de trás             
  circle(target.x, target.y, target.w);
    }
